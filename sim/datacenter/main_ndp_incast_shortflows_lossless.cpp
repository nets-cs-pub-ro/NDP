...
int main(int argc, char **argv) {
    ...
    eventlist.setEndtime(timeFromSec(0.3));
    ...
    NdpSinkLoggerSampling sinkLogger = NdpSinkLoggerSampling(timeFromMs(0.1), eventlist);
...
#ifdef FAT_TREE
    FatTreeTopology* top = new FatTreeTopology(no_of_nodes, memFromPkt(10), &logfile, 
					       &eventlist,ff,LOSSLESS_INPUT,0);
#endif
	...
	for (unsigned int dst_id = 0;dst_id<destinations->size();dst_id++){
		...
		routeout->add_endpoints(ndpSnk, ndpSrc);
