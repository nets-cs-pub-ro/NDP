...
int main(int argc, char **argv) {
    ...
    int no_of_conns = DEFAULT_NODES, cwnd = 30, no_of_nodes = DEFAULT_NODES;
...
#ifdef OV_FAT_TREE
    OversubscribedFatTreeTopology* top = new OversubscribedFatTreeTopology(&logfile, &eventlist,ff,COMPOSITE);
#endif
	...
	for (unsigned int dst_id = 0;dst_id<destinations->size();dst_id++){
		...
		ndpSnk = new NdpSinkTransfer(pacers[dest]);
	...
	for (int i = 0; i < r->size(); i++) {
		...
		stripped[hop] += q->num_stripped();
		bounced[hop] += q->num_bounced();
		counts[hop] += q->num_packets();				
		q->_num_stripped = 0;
		q->_num_packets = 0;
		q->_num_bounced = 0;
		hop++;
    ...
    for (int i = 0; i < 10; i++)
	cout << "Hop " << i << " Count " << counts[i] << " Stripped " << stripped[i] << " Bounced " << bounced[i] << endl;
