...
int main(int argc, char **argv) {
	...
	if (!strcmp(argv[i],"-o")){
	    ...
	    i++;
	} else if (!strcmp(argv[i],"-ndpq")){
	    ndpqs = memFromPkt(atoi(argv[i+1]));
	    i++;
    ...
    NdpTunnelRtxTimerScanner ndpRtxScanner(timeFromMs(10), eventlist);
    ...
    list <NdpTunnelSrc*> ndp_srcs;
    // initialize all sources/sinks
    NdpTunnelSrc::setMinRTO(10000); //increase RTO to avoid spurious retransmits
	...
	for (unsigned int dst_id = 0;dst_id<destinations->size();dst_id++){
	    ...
	    ndpSrc->set_queue_size(ndpqs);
	    ndp_srcs.push_back(ndpSrc);
	    ndpSnk = new NdpTunnelSink(eventlist, 1 /*pull at line rate*/);
		...
		tcpSrc->setName("TCP"+ntoa(i)+ntoa(connection)); logfile.writeName(*tcpSrc);
		tcpSnk = new TcpSink();
		//tcpSnk = new TcpSinkTransfer();                                       
		tcpSnk->setName("TCPSink"+ntoa(i)+ntoa(connection)); logfile.writeName(*tcpSnk);
		...
		tcpSrc->connect(*routeout,*routein,*tcpSnk,0);
